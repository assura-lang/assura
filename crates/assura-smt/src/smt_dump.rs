...
pub fn dump_smt_queries(typed: &TypedFile) -> Vec<SmtQuery> {
    ...
    for decl in &typed.resolved.source.decls {
        ...
        for clause in clauses {
            // Dump only solver-oriented clause kinds (same set as before; skip requires/effects/etc.)
            if !matches!(
                clause.kind,
                ClauseKind::Ensures
            ...
            let kind_str = crate::verify_labels::clause_kind_label(&clause.kind);
            ...
            script.push_str(&format!(
                "; Context: {}\n",
                crate::verify_labels::clause_desc(&name, &clause.kind)
            ));
