    ...
    for decl in &typed.resolved.source.decls {
        ...
        for clause in clauses {
            ...
            for req in &requires_exprs {
                crate::cvc5_backend::collect_vars(req, &mut vars);
            }
            crate::cvc5_backend::collect_vars(&clause.body, &mut vars);
            ...
            for req in &requires_exprs {
                if let Some(smt) = crate::cvc5_backend::expr_to_smtlib(req) {
                    script.push_str(&format!("; requires\n(assert {smt})\n"));
                }
            }

            if let Some(smt) = crate::cvc5_backend::expr_to_smtlib(&clause.body) {
