...
fn rust_type_to_assura(ty: &str) -> &str {
    let trimmed = ty.trim();
    match trimmed {
...
fn clause_to_json(clause: &assura_rust_analyzer::ContractClause, kind: &str) -> serde_json::Value {
    serde_json::json!({
        "kind": kind,
        "body": clause.body,
...
fn analyze_function_body(func: &syn::ItemFn, source: &str, focus: &[&str]) -> Vec<InferSuggestion> {
...
fn extract_block_text(block: &syn::Block, _source: &str) -> String {
    use quote::ToTokens;
