...
/// Add a `src/main.rs` binary entry that calls the primary contract's `check`.
fn inject_bin_main(
    ...
    let bin_name = primary
        ...
        .collect::<String>();
    let main_rs = format!(
        r#"// Generated by assura build --bin
use {pkg}::{mod_name};

fn main() {{
    let x: i64 = std::env::args()
        .nth(1)
        .and_then(|s| s.parse().ok())
        .unwrap_or(42);
    let r = {mod_name}::check(x);
    println!("{{r}}");
}}
"#
    );
