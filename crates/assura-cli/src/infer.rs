...
"#;
    ...
    #[test]
    fn clean_public_fn_has_sig_for_minus_o_fallback() {
        // Heuristic suggestions are empty for pure arithmetic; -o must still
        // be able to extract public signatures for skeleton binds.
        let source = "pub fn add(a: i64, b: i64) -> i64 { a + b }\nfn private() {}\n";
        let sigs = extract_rust_fn_signatures(source);
        let public: Vec<_> = sigs.iter().filter(|s| s.is_pub).collect();
        assert_eq!(public.len(), 1);
        assert_eq!(public[0].name, "add");
        let mut out = String::new();
        generate_bind_skeleton("lib", public[0], &mut out);
        assert!(out.contains("bind \"lib::add\" as add"));
    }
}
