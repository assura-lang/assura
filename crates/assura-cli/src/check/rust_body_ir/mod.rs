...
/// Lift if/match out of binary operands so multi-block encode can fire.
/// `(if c { a } else { b }) ⊕ r` → `if c { a ⊕ r } else { b ⊕ r }` (and right/match).
    ...
    match expr {
        ...
        syn::Expr::MethodCall(m) => {
            ...
            match (method.as_str(), m.args.len()) {
                ...
                // Typed lit (incl. negative via mask); path params ≤32
                // unsigned direct or signed via bit-pattern map.
                ("leading_zeros", 0) => {
                    if let Some((v, bits)) = lit_int_i64_bits(&m.receiver) {
                        let mask = if bits >= 64 {
                ...
                // Const peep; variable unsigned path ≤64 (ladder ≤16; binsearch 17..=64).
                ("isqrt", 0) => {
                    if let Some(v) = lit_int_i64(&m.receiver) {
                        if v < 0 {
                            return None;
