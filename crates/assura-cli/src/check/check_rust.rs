...
pub(crate) fn run_check_rust(
    ...
    if output_mode == OutputMode::Json {
        let summary = serde_json::json!({
            ...
            "results": all_results,
            "policy": "check-rust proves annotations against co-located .ir or encoded Rust bodies (int/bool arith/&&/||, if/else/match, multi-let, abs/min/max/clamp/saturating/abs_diff/pow, is_multiple_of, into/as/default/MINMAX, PartialOrd/borrow/deref, as_ref/not)",
        });
