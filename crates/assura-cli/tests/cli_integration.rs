...
//!
//! `check-rust` body-IR encode/CE pairs live in `check_rust_body_ir.rs` (#1352).
...
#[test]
fn check_unresolved_import_does_not_verify_unknown_name() {
    let tmp = unique_temp("assura_check_unresolved_import_name");
    let _ = std::fs::remove_dir_all(&tmp);
    std::fs::create_dir_all(&tmp).unwrap();
    ...
    assert_eq!(
        out.status.code(),
        Some(1),
        "unknown y must not pass: stdout={stdout} stderr={stderr}"
    );
    let v: serde_json::Value = serde_json::from_str(stdout.trim()).unwrap_or_else(|e| {
    ...
    assert!(
        ...
        "must not verify y == y behind a missing import: {v}"
    );
    let _ = std::fs::remove_dir_all(&tmp);
}
