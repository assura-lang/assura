...
`count_ones`/`count_zeros`/`trailing_zeros`/`leading_zeros` (≤32; signed via
bit-pattern map). `signum` is nestable in arith (clamp to [-1, 1]). Top-level
...
`reverse_bits`. Bodies that cannot be modeled report `body_not_modeled` and
exit **1** (including SMT skipped/checked soft passes). Do not treat
empty/skipped SMT as proof.
