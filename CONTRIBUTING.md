...
`swap_bytes` (≤32; signed via bit-pattern map), and `rem_euclid`/`div_euclid`/
`next_multiple_of` with a positive const or `NonZeroU*` path-param divisor
(`.get()` peels). `signum` is nestable in arith (clamp to [-1, 1]). Top-level
